...
namespace BlazorBoilerplate.Server.Code
{
    public class BlobManager
    {
        ...
        public List<string> GetBlobDirectoryList(string blobPrefix)
        {
            var result = blobContainer.GetDirectoryReference(blobPrefix);
            List<string> blobDirectoryList = result.ListBlobs().OfType<CloudBlobDirectory>()
                .Select(b => b.Uri.ToString().Split(blobPrefix).LastOrDefault().Replace(@"/", string.Empty)).ToList();
            return blobDirectoryList;
        }
